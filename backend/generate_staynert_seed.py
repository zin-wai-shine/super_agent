...
user_id = 'be2eb7bf-1e1e-4f59-9ebc-7092e2da0bbe'
...
sql = "-- Refresh Staynert data at server\n"
sql += "BEGIN;\n"
sql += f"DELETE FROM saved_listings WHERE listing_id IN (SELECT id FROM listings WHERE agent_id = '{agent_id}');\n"
sql += f"DELETE FROM appointments WHERE listing_id IN (SELECT id FROM listings WHERE agent_id = '{agent_id}');\n"
sql += f"DELETE FROM media WHERE listing_id IN (SELECT id FROM listings WHERE agent_id = '{agent_id}');\n"
sql += f"DELETE FROM listings WHERE agent_id = '{agent_id}';\n"

# Generate 30 listings by cycling through templates
for i in range(30):
    item = listings_templates[i % len(listings_templates)]
    l_id = str(uuid.uuid4())
    # Add some variation to the title for uniqueness
    title = f"{item[0]} {i+1}" if i >= len(listings_templates) else item[0]
    
    sql += f"INSERT INTO listings (id, created_at, updated_at, agent_id, created_by, title, description, listing_type, property_type, price, bedrooms, bathrooms, area, address, district, province, station_id, station_name, is_published, is_featured, view_count, availability_status) VALUES ('{l_id}', NOW(), NOW(), '{agent_id}', '{user_id}', '{title}', '{item[1]}', '{item[2]}', '{item[3]}', {item[4]}, {item[5]}, {item[6]}, {item[7]}, '{item[8]}', '{item[9]}', '{item[10]}', '{item[11]}', '{item[12]}', {item[13]}, {item[14]}, {item[15]}, '{item[16]}');\n"
    
    # generate 5 images per listing to ensure "contain image" requirement
    used_images = random.sample(images, 5)
    for j in range(5):
        m_id = str(uuid.uuid4())
        img_url = used_images[j]
        room_types = ['Living Room', 'Bedroom', 'Kitchen', 'Bathroom', 'Additional Photos']
        rt = room_types[j % len(room_types)]
        sql += f"INSERT INTO media (id, created_at, listing_id, type, url, thumbnail, caption, room_type, sort_order) VALUES ('{m_id}', NOW(), '{l_id}', 'image', '{img_url}', '{img_url}', '{rt}', '{rt}', {j});\n"

sql += "COMMIT;\n"

with open("/Users/zinwaishine/Desktop/super_real_estate/backend/seed_staynert_server.sql", "w") as f:
    f.write(sql)
