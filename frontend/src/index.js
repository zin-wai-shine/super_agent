...
const getTenantInfo = () => {
    try {
        ...
        const cached = sessionStorage.getItem(`tenantConfig_${hostname}`);
        if (cached) {
            const config = JSON.parse(cached);
            const agent = config?.agent;
            const isMainDomain = config?.is_main_domain;
            ...
            const primaryColor = agent?.theme?.primary_color || agent?.theme?.primaryColor || '#ea580c';
            ...
            return { logoUrl, siteTitle, primaryColor };
        }
    } catch (e) {
        console.error('ErrorBoundary: Failed to parse tenant config', e);
    }
    return { logoUrl: null, siteTitle: 'Super Real Estate', primaryColor: '#ea580c' };
};
...
class ErrorBoundary extends React.Component {
    ...
    render() {
        if (this.state.hasError) {
            const { logoUrl, siteTitle, primaryColor } = getTenantInfo();
            return (
                <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-white dark:bg-dashboard-dark p-6 text-center animate-in fade-in duration-700">
                    ...
                    <div className="relative z-10 w-full max-w-md animate-fade-up">
                        ...
                        <div className="flex justify-center mb-8">
                            <div className="flex items-center space-x-3 bg-white/50 dark:bg-dashboard-card/50 backdrop-blur-sm px-4 py-2 rounded-full border border-white dark:border-white/10 shadow-sm">
                                {logoUrl ? (
                                    <img 
                                        src={logoUrl} 
                                        alt={siteTitle} 
                                    ...
                                    <div 
                                        className="w-8 h-8 rounded-lg flex items-center justify-center shadow-lg text-white font-bold text-lg"
                                        style={{ backgroundColor: primaryColor }}
                                    >
                                        {siteTitle?.charAt(0) || 'S'}
                                    </div>
                                )}
                                <span className="font-bold text-gray-900 dark:text-white tracking-tight">{siteTitle}</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-white/70 dark:bg-dashboard-card/70 backdrop-blur-xl border border-white/50 dark:border-white/5 rounded-[32px] p-8 md:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
                            ...
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-3 tracking-tight">{i18n.t('error.somethingWentWrong')}</h1>
                            ...
                            <button
                                ...
                                className="group relative flex items-center justify-center w-full px-8 py-4 text-white font-bold rounded-full transition-all duration-300 shadow-xl hover:brightness-95 transform hover:-translate-y-1 overflow-hidden"
                                style={{ backgroundColor: primaryColor }}
                            >
                        ...
                        <div className="mt-8 text-center">
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{i18n.t('error.protectionActive')}</span>
                        </div>
                    </div>
