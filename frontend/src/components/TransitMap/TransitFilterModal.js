...
import { MagnifyingGlassIcon, XMarkIcon, CheckIcon, MapIcon, ChevronLeftIcon } from '@heroicons/react/24/outline';
...
const TransitFilterModal = ({
    ...
    return (
        <Modal
            ...
            className="!w-full !h-full sm:!w-[96vw] sm:!h-[92vh] max-w-none rounded-none sm:rounded-[22px] overflow-hidden"
        >
            <div className="flex-1 flex flex-col overflow-hidden min-h-0 h-full">
                ...
                <div className={`${showMapOnMobile ? 'hidden sm:flex' : 'flex'} w-full sm:w-[450px] flex-col bg-white min-h-0 border-r border-gray-100 flex-1 min-w-0`}>
                    {/* Header: Fixed */}
                    <div className="p-6 border-b border-gray-50 flex-shrink-0 bg-white">
                        ...
                        {/* Selected Pills — full width (no left/right padding), 2 rows, scroll left/right when more */}
                        {selectedIds.length > 0 && (
                            <div className="-mx-6 w-[calc(100%+3rem)] overflow-y-hidden overflow-x-auto scrollbar-hide py-3" style={{ maxHeight: '6rem' }}>
                                <div className="inline-grid grid-flow-col grid-rows-2 auto-cols-max gap-x-3 gap-y-2 pb-0.5 pl-6">
                                    {selectedIds.map(id => {
                            ...
                            filteredGroups.map(group => {
                                ...
                                return (
                                    <div key={group.name} className="mt-6 first:mt-0 mb-10 last:mb-20">
                                        <div className="flex items-center justify-between mb-4 bg-white z-10 sticky top-0 pt-4 pb-3 -mx-6 px-6 border-t border-b border-gray-200">
                ...
                <div className={`${showMapOnMobile ? 'flex flex-col min-h-0' : 'hidden sm:flex flex-col min-h-0'} flex-1 relative bg-gray-50 overflow-hidden min-w-0`}>
                    {/* Back to list — mobile only when map is shown */}
                    {showMapOnMobile && (
                        <button
                            type="button"
                            onClick={() => setShowMapOnMobile(false)}
                            className="sm:hidden absolute top-3 left-3 z-[100] flex items-center gap-2 px-3 py-2 rounded-full bg-white/95 backdrop-blur shadow-md border border-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-50"
                        >
                ...
                {/* Footer: always visible (list and map view) so it's not removed on mobile */}
                <div className="p-4 sm:p-6 border-t border-gray-100 flex-shrink-0 bg-white flex items-center justify-between gap-4 sm:gap-6 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
                    <div className="flex items-center gap-4">
                        <div>
                            <p className="text-[13px] sm:text-[11px] font-black text-gray-900 uppercase tracking-widest leading-none">
                                {selectedIds.length} stations
                            </p>
                            <p className="text-[11px] sm:text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1.5">
                                selected
                            </p>
                        </div>
                        {selectedIds.length > 0 && (
                            <button
                                ...
                                className="text-[11px] sm:text-[9px] font-black text-red-500 hover:text-red-600 transition-colors border-l border-gray-100 pl-4 py-1"
                            >
                                Clear all
                            </button>
                        )}
                    ...
                    <button
                        onClick={handleApply}
                        className="flex-1 max-w-[200px] py-4 bg-primary-600 text-white text-[13px] sm:text-xs font-black rounded-full hover:bg-primary-700 transition-all hover:shadow-xl hover:shadow-primary-600/30 active:scale-95"
                    >
                        Done
                    </button>
                </div>
