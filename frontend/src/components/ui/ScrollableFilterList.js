    ...
    return (
        <div className="flex flex-col gap-2">
            {!isExpanded ? (
                ...
                >
                    <span className="text-[13px] font-normal text-gray-900 truncate">{displayLabel}</span>
                    <ChevronDownIcon className="w-4 h-4 text-gray-700 stroke-[2] flex-shrink-0" />
                </button>
            ) : (
                /* Expanded: same box is the search input, then list below */
                <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="relative group flex items-center min-h-[48px] rounded-full border border-gray-800 bg-white px-4">
                        ...
                        <input
                            ...
                            className="flex-1 min-w-0 py-2.5 text-[13px] font-normal text-gray-900 placeholder-gray-500 bg-transparent border-none focus:outline-none focus:ring-0"
                            aria-label="Search"
                        />
                        <button
                            type="button"
                            onClick={handleToggle}
                            className="p-1 rounded-full hover:bg-gray-100 text-gray-700 flex-shrink-0"
                    ...
                    <div className="flex flex-col max-h-[280px] overflow-y-auto custom-scrollbar-thin pr-1 py-1 gap-1">
                        <button
                            ...
                            className={`flex items-center w-full px-3 py-2.5 rounded-full border transition-all text-left ${!selectedId ? 'bg-white border-gray-800 text-gray-900' : 'bg-white border-gray-200 hover:border-gray-400 text-gray-600'
                                }`}
                        >
                        ...
                        {filteredItems.map((item) => (
                            <button
                                ...
                                className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-full border transition-all text-left group ${selectedId === item.id ? 'bg-white border-gray-800' : 'bg-white border-gray-200 hover:border-gray-400'
                                    }`}
                            >
                                <div className="flex-shrink-0">
                                    {item.image ? (
                                        <img
                                            ...
                                            alt=""
                                            className={`w-9 h-9 object-cover ${item.isAvatar ? 'rounded-full' : 'rounded-lg'} border border-gray-200`}
                                        />
                                    ) : item.icon ? (
                                        <div className={`w-9 h-9 flex items-center justify-center bg-gray-100 text-gray-600 ${item.isAvatar ? 'rounded-full' : 'rounded-lg'}`}>
                                            {item.icon}
                                        </div>
                                    ) : (
                                        <div className={`w-9 h-9 flex items-center justify-center bg-gray-100 text-gray-500 ${item.isAvatar ? 'rounded-full' : 'rounded-lg'}`}>
                                            <span className="text-[11px] font-normal">{item.name?.substring(0, 2)}</span>
                                        </div>
                                    )}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className={`text-[13px] font-normal truncate ${selectedId === item.id ? 'text-gray-900' : 'text-gray-700 group-hover:text-gray-900'}`}>
                                        {item.name}
                                    </div>
                                </div>
                            </button>
