...
export const ListingDetailView = ({ id: propId, isModal = false, onTitleChange, onHeaderLeadingChange, onBookingOpenChange, onClose, onOpenGallery, setHideLayout }) => {
    ...
    const renderBookingContent = (isDesktopPage = false) => {
        ...
        return (
            <div
                className={`flex flex-col ${isDesktopPage ? '' : 'overflow-hidden h-full sm:rounded-[32px] rounded-none bg-white dark:bg-dashboard-dark'}`}
            >
                {/* Header */}
                {!success && (
                    <div
                        className={`sticky top-0 z-[80] flex items-center shrink-0 transition-all duration-300 h-[76px] lg:h-auto py-0 lg:py-6 px-4 md:px-8 lg:px-20 bg-white/95 dark:bg-dashboard-dark/95 backdrop-blur-md ${isDesktopPage ? '-mx-4 md:-mx-8 lg:-mx-20' : 'border-b transition-shadow'} ${(!isDesktopPage && bookingScrolled) ? 'border-gray-100 dark:border-white/10 shadow-sm' : 'border-transparent'}`}
                    >
                            ...
                            <>
                                ...
                                {!bookingId && relatedListings.length > 0 && (
                                    <div className="hidden md:block w-full py-12 border-b border-gray-100 dark:border-white/10">
                                        ...
                                        {isMapView ? (
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-6 px-4 md:px-0 lg:px-0">
                                            ...
                                            <div className="flex flex-nowrap gap-4 overflow-x-auto overflow-y-hidden scrollbar-hide snap-x snap-mandatory px-4 md:px-0 lg:px-0 sm:gap-6" style={{ WebkitOverflowScrolling: 'touch' }}>
                                                {relatedListings.map((related) => (
                                                    <div
                                                        key={related.id}
                                ...
                                {!isDesktopView && (
                                    <>
                                        ...
                                        >
                                            ...
                                            />
                                            <div
                                                className={`absolute bottom-0 left-0 right-0 z-[100] bg-white dark:bg-dashboard-card transition-all duration-[500ms] [transition-timing-function:cubic-bezier(0.32,0.72,0,1)] will-change-transform rounded-t-[20px] h-[70vh] flex flex-col overflow-hidden
